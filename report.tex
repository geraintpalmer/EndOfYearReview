...
Social vs selfish rewards corresponds to node receiving rewards according to the system's state vs receiving rewards from just its own state.
...
The performance measures of the queueing network models will be used to
...
Section~\ref{sec:litreview} will review the relevant literature, Section~\ref{sec:analyticalmodels} will discuss methods of analytically investigating queueing networks, and Section~\ref{sec:deadlock} will discuss deadlocking properties of queueing networks.
...
where $P_i(k_i)$ is the probability of node $i$ containing $k_i$ customers, and $P(k_1, k_2, \dotsc, k_M)$ represents the probability of the system being in the state $(k_1, k_2, \dotsc, k_M)$, having $k_j$ customers at node $j$ for all $j$.
...
Balking is defined as a customer arriving but deciding not to join the queue,
...
The above equations yield $P_1(3) \approx 0.10541$, $P_2(1) \approx 0.22172$ and $P_3(1) \approx 0.29827$.
And so:
...
This method is entirely equivalent to the method described in the next subsection, Markov chains.
...
\begin{figure}[H]
    ...
    \caption{A two-node queueing network with finite queues.}
    \label{fig:2nodesQs}
\end{figure}
...
Table~\ref{tab:eventtriggers} below shows some of the possible conditional events that arise from other events.

 \begin{table}[H]
 \begin{center}
\begin{tabular}{ | p{4cm} | p{4cm} | p{7.8cm} | }
    \hline
    \begin{center}\textbf{\textit{Event}}\end{center} & \begin{center}\textbf{\textit{Conditional Event}}\end{center} & \begin{center}\textbf{\textit{Reason}}\end{center} \\
    \hline
    ...
    \hline
\end{tabular}
 \caption{Table listing possible conditional event triggers.}
 \label{tab:eventtriggers}
 \end{center}
 \end{table}
...
The Nodes are where \texttt{Individuals} queue for service, receive service, and remain when blocked.
...
Here most the simulation process is carried out in the command `have event'.
If the next active node is an \texttt{ArrivalNode} then the following pseudocode describes the method:

\begin{algorithm}[H]
    \DontPrintSemicolon
    Create a new \texttt{Individual()}\;
    Determine this individual's next node\;
    \eIf{enough capacity at next node}{
        Join queue at next node\;
    }{
        The individual is lost to the system\;
    }\;
\end{algorithm}

\texttt{ExitNode} will never have an event, as its next event date  is always set to infinity.
For all other \texttt{Nodes}, the `have event' command is described in the pseudocode below:

\begin{algorithm}[H]
    \DontPrintSemicolon
    next individual $\leftarrow$ individual that is finishing service\;
    Determine next individual's next node\;
    \eIf{enough capacity at next node}{
        release next individual\;
    }{
        block next individual\;
    }\;
\end{algorithm}

where the command release acts as follows:

\begin{algorithm}[H]
    \DontPrintSemicolon
    Move individual to next node\;
    \If{queue length at next node = 0}{
        start service\;
    }\;
    \If{any individuals need to join this queue}{
        release blocked individual\;
    }\;
...
Notice that the release command is recursive, and so the process of unblocking individuals can continue until no more individuals can be unblocked.


\section{Deadlocking Properties of Open Restricted Queueing Networks}\label{sec:deadlock}
...
\begin{figure}[H]
\begin{subfigure}[b]{0.5\textwidth}
  \includestandalone[width=0.9\textwidth]{images/transientdeadlock}vertices
...
\begin{theorem}
    The existence of a knot in the state digraph is equivalent to the existence of a vertex with an out-edge but without a path to a sink.
\end{theorem}
...
It is shown in \cite{stewart09} that for a discrete transition matrix of the form $P = \bigl(\begin{smallmatrix} T & U\\ 0 & V \end{smallmatrix} \bigr)$ then the expected number of time steps until absorption starting from state $i$ is the $i\text{th}$ element of the vector
\begin{equation}
  (I - T)^{-1}e
\end{equation}
